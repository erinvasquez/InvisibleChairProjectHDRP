...
public class Maze : MonoBehaviour {
    ...
    public void Initialize(Vector3 startPos, Vector3 rotation, Vector3 scale, float genStepDelay, float doorProb) {
        transform.rotation = Quaternion.Euler(rotation.x, rotation.y, rotation.z);
        transform.position = startPos;

        transform.localScale = scale;
        mazeScale = scale.x; // assuming x y z all the same

        generationStepDelay = genStepDelay;
        doorProbability = doorProb;
    ...
    /// <returns></returns>
    public IEnumerator Generate(IntVector2 size) {
        mazeSize = size;
    ...
    private MazeCell CreateCell(IntVector2 coordinates) {
        ...
        // Get the cell ready
        // (Put this in an Initialize() method in MazeCell)
        newCell.coordinates = coordinates;
        newCell.name = "Maze Cell " + coordinates.x + ", " + coordinates.z;
        newCell.transform.parent = transform;
        newCell.transform.Rotate(transform.localRotation.eulerAngles);

        // scale for our new scale is already affected by our local scale here, so divide by it
        newCell.transform.localScale = new Vector3(mazeScale / transform.localScale.x, mazeScale / transform.localScale.y, mazeScale / transform.localScale.z);

        // Coordinate X should be set as
        // Subtract maze size x, half it, and move it over
